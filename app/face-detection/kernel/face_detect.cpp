...
int weak_classifier(int stddev, uint18_t coord[12], int haar_counter, int w_id) {
...
#pragma HLS pipeline
            ...
            /* Read the values corresponding to the 12 locations from _II which
             * is the copy of integral image window */
            ...
            s = weak_classifier(stddev, coord, haar_counter, w_id);
...
#pragma HLS array_partition variable = SII complete dim = 0
    ...
    const int scale_height_list[12] = {200, 167, 139, 116, 96, 80, 67, 56, 47, 39, 32, 27};
    const int scale_width_list[12] = {267, 222, 185, 154, 129, 107, 89, 74, 62, 52, 43, 36};
    const int window_height_list[12] = {29, 35, 41, 50, 60, 72, 86, 103, 124, 149, 178, 214};
    const int window_width_list[12] = {29, 35, 41, 50, 60, 72, 86, 103, 124, 149, 178, 214};

    int sum_col = scale_height_list[factor];
    int sum_row = scale_width_list[factor];
    int h = window_height_list[factor];
    int w = window_width_list[factor];
...
#pragma HLS unroll
            ...
            I[WINDOW_SIZE - 1][2 * WINDOW_SIZE - 1] = image_data;
...
#pragma HLS unroll
            ...
            /* Pass the Integral Image Window buffer through Cascaded Classifier.
             * Only pass when the integral image window buffer has flushed out the initial garbage data */
