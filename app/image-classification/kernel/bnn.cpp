...
// -----------------------------------------------------------------------
void bin_conv(Word wt_mem[CONVOLVERS][C_WT_WORDS],
              NormComp nc,
    ...
    LOOP_PHASES:for (ap_uint<10> p = 0; p < n_phases; p += images_per_phase) {
        ...
        LOOP_WORDS_IN_PHASE:for (ap_uint<8> count = 0; count < WORDS_PER_PHASE + images_per_phase; ++count) {
            ...
            if (wrd == 0) {
                ...
                // -------------------------------------------------------------------
                // Load params
                // Each word contains CONV_W_PER_WORD weight filters packed into the
                // first 63 bits, the last bit is unused. Wts are stored in
                // output-major order.
                // -------------------------------------------------------------------
            ...
            if (wrd != words_per_image) {
                LOOP_CONVOLVER_LOAD:for (IdxType m = 0; m < CONVOLVERS; ++m) {
                    ...
                    for (IdxType bank = 0; bank < CONV_BANKS; ++bank) {
                        ...
                        word_buffer[m][bank][0]             = (bank == 0) ?              TwoBit(0) : encode_bit(word[ap_uint<6>(bank * BANK_WIDTH - 1)]);
                        word_buffer[m][bank][CONV_COLS - 1] = (bank == CONV_BANKS - 1) ? TwoBit(0) : encode_bit(word[ap_uint<6>(bank * BANK_WIDTH + BANK_WIDTH)]);
                    }
                }
            }
            ...
            LOOP_CONVOLVERS:for (IdxType m = 0; m < CONVOLVERS; ++m) {
                ...
                process_word(word_buffer[m], old_word_buffer[m], lb, rb,
                             line_buffer[m], conv_params[m], conv_out_buffer[m],
                             log_width, words_per_image, wrd);
            }  // CONVOLVERS
...
#pragma HLS unroll
    ...
    LOOP_BATCH_NORM:for (ap_uint<6> w = 0; w < words_per_image; ++w) {
        ...
        if (norm_mode == 1) {
            ...
            // For log_width > 3 we can just assign the word, but log_width = 3
            // means width = 8, which means pooled width = 4, which is only 16
            // bits, which is less than 1 Word. So each time we get here we only
            // have 16 bits, meaning we have to accumulate four of these 16-bit
            // batches before writing a word out.
...
// -----------------------------------------------------------------------
void fp_conv(Word wt_mem[CONVOLVERS][C_WT_WORDS],
...
void bin_dense(const Word wt_mem[CONVOLVERS][C_WT_WORDS],
...
void image_cls(Word wt_i[WT_WORDS],
    ...
    if (layer_type == LAYER_CONV1) {
        fp_conv(wt_mem, kh_mem, dmem, d_i_idx, d_o_idx, kh_index, o_index, n_outputs);
        kh_index += n_outputs;
        o_index += n_outputs;
        ...
        LOOP_IMG_BATCH:for (IdxType i = 0; i < n_outputs; ++i) {
            ...
            bin_conv(wt_mem, nc, dmem, d_i_idx, d_o_idx, n_inputs, o_index,
                     i == 0 ? 1 : 0,  // new_batch
        ...
        bin_dense(wt_mem, kh_mem, dmem, layer_type, d_i_idx, d_o_idx, o_index, n_inputs, n_outputs);
        o_index += n_outputs;
    }  // layer_type
